...
Double_t LookUp::GetDistance(Double_t InitialE, Double_t FinalE, Double_t StepSize, Int_t Max)
{
  ...
  for(int i =0; i<Max; i++){
    ...
    if((fabs(E-FinalE)/FinalE<tolerance) || (E<FinalE)) break;
  }
...
void LookUp::GetStraggle(Double_t z_dist, Double_t &z, Double_t &y) {
  if(z_dist > Range[Range.size()-1]) {
    cout<<"Particle is out of range at GetStraggle!!"<<endl;
  } else {
    int index = distance(Range.begin(), lower_bound(Range.begin(), Range.end(), z_dist));
    z = longStrag[index];
    y = latStrag[index];
  }
}
